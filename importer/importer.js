...
function getJsonStringFromData(){
    ...
    while((match = findTechPattern.exec(dataFeed)) != null){
        ...
        if(!skipTechs.includes(match[0])){
            ...
            techDetails.name = match[0];
            jsonData.techs.push({tech:techDetails}); // Save the tech name to JSON object
        } // Check for skipped Techs
    }
}

function getJsonDataFeed(targetID){
    getJsonStringFromData();
    document.getElementById(targetID).innerHTML = JSON.stringify(jsonData);
